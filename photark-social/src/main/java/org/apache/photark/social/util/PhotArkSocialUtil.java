...
public class PhotArkSocialUtil {
    ...
    private static final Logger logger = Logger.getLogger(PhotArkSocialUtil.class.getName());
     ...
     */
    public static Node getSocialDataRoot(JCRRepositoryManager repositoryManager) throws PhotarkRuntimeException {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Error retrieving social data from the repository :" + e.getMessage(), e);
            throw new PhotarkRuntimeException("Error retrieving social data from the repository :" + e.getMessage(), e);
        }
     ...
     * 
     * @param repositoryManager JCRRepositoryManager object
     * @param username UserId of the Person
     * @param create If true, creates the node when it doesn't exist; If false,
     *            return null if the node doesn't exist
     * @return the node for the given user name
     * @throws PhotarkRuntimeException
     */
    public static Node getPersonRootNode(JCRRepositoryManager repositoryManager, String username, boolean create)
        throws PhotarkRuntimeException {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Error retrieving person root node from the repository :" + e.getMessage(), e);
            throw new PhotarkRuntimeException(
                                              "Error retrieving person root node from the repository :" + e
                                                  .getMessage(),
                                              e);
        }
     ...
     * @throws PhotarkRuntimeException
     */
    public static Node getPersonProfileRootNode(JCRRepositoryManager repositoryManager, String username, boolean create)
        throws PhotarkRuntimeException {
        Session session = null;
        Node personRootNode = null;
        ...
        try {
            session = repositoryManager.getSession();
            personRootNode = getPersonRootNode(repositoryManager, username, create);
            if (personRootNode.hasNode(JCR_PROFILE_ROOT_NODE)) {
                personProfileRootNode = personRootNode.getNode(JCR_PROFILE_ROOT_NODE);
            } else {
                if (create) {
                    personProfileRootNode = personRootNode.addNode(JCR_PROFILE_ROOT_NODE);
                    session.save();
                }
            }
        } catch (LoginException e) {
            ...
            logger.log(Level.SEVERE, "Error retrieving person profile node from the repository :" + e.getMessage(), e);
            throw new PhotarkRuntimeException(
                                              "Error retrieving person profile node from the repository :" + e
                                                  .getMessage(),
                                              e);
        }
